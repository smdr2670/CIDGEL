...
\If{${a}_{i} - {b}_{i} \notin \{{a}_{j} - {b}_{j} : \underline{x}^{a_{j}} - {x}^{b_{j}} \in \mathcal{G} \}  $}
...
The breadth-first search now needs a degree compatible Groebner basis as an input. This can be achieved by applying the Buchberger Algorithm with a degree compatible monomial, for example the \textit{grlex} order. After that it is required that the Groebner basis is checked if its is the only degree compatible basis. %see only degree compatible Groebner basis
