...
\While{$i\leq m$  \textbf{and} division\_occured = false}
...
First, with respect to the lex-order, applying algorithm \ref{alg:division} gives the expression : $f = x^{2}(xy^{2}+z) + 0(y^{2}-1) + 0.$ \\
...
The solution of the Ideal Membership Problem needs a certain generating set for an ideal $I$. It would be helpful if the remainder $r$ on division is uniquely determined and the condition $ r = 0~$ is equivalent to the ideal membership.
...
\Repeat
\State $G'\gets G $
\For{\textbf{each} pair $f_{i}$ and $f_{j}$ in $G$ , $i\neq j$ }
...
Note that the ring has 3 variables so that the Gröbner fan can be plotted in the positive orthant $ \mathbb{R}^{3}_{+}$. \\
...
\item
$\left( 0,2,0\right) \cdot \left( a,b,c\right) \geq \left( 0,0,1\right) \cdot \left( a,b,c\right) $ \textbf{or} $2b\geq c$ 
\item
$\left( 1,0,0\right) \cdot \left( a,b,c\right) \geq \left( 0,1,0\right) \cdot \left( a,b,c\right) $ \textbf{or} $a\geq b$ 
...
\begin{figure}
    ...
    \end{subfigure}
    \caption{Gröbner fans for example \ref{ex:groebnerfan}}
...
Example \ref{ex:groebnerfan} illustrates clearly that an arbitrary non-negative weight vector can be selected and if the vector lies in a certain cone, the corresponding Gröbner basis will match with respect to this weight vector.  


This strategy is reasonable for small examples like above.
In general, the whole Gröbner fan can be computed with the Gröbner walk \cite{coxOshea}.\\
